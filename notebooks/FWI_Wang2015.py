...
def calc_ffmc(tmeanC, wind_kmhr, RH, precip_mm, ffmc_prev):
    ...
    if precip_mm > 0.5:
        ...
        if Mo > 150.0:
            Mo = (
                Mo
                + 42.5 * rainfall * np.exp(-100.0 / (251.0 - Mo)) * (1.0 - np.exp(-6.93 / rainfall))
            ) + (0.0015 * (Mo - 150.0) ** 2.0) * (rainfall ** 0.5)
        else:  # Mo<= 150.0
            Mo = Mo + 42.5 * rainfall * np.exp(-100.0 / (251.0 - Mo)) * (
                1.0 - np.exp(-6.93 / rainfall)
            )
    ...
    edry = (
        ...
        + 0.18 * (21.1 - tmeanC) * (1.0 - 1.0 / np.exp(0.1150 * RH))
    )
    ...
    ewet = (
        ...
        + 0.18 * (21.1 - tmeanC) * (1.0 - 1.0 / np.exp(0.115 * RH))
    )
...
@numba.vectorize(['float64(float64, float64)', 'float32(float32, float32)'], nopython=True)
